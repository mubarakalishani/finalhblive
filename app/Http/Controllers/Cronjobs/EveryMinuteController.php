...
class EveryMinuteController extends Controller
{
        ...
        foreach ($offersPendingCleared as $offer) {
            ...
            if (!$completedOfferExist) {
                ...
                if($uplineId > 0)
                {
                    ...
                    $uplineToReward->increment('balance', $offer->upline_commision );
    ...
    protected function checkAllWithdrawals(){
        ...
        foreach ($pendingWithdrawals as $pendingWithdrawal) {
            ...
            if ( $sumOfWithdrawals > ((($user->total_earned + $user->earned_from_referrals)) + ((($user->total_earned + $user->earned_from_referrals)/100)) * 10)) {
            ...
            $offerRecords = OffersAndSurveysLog::where('payout', '>=', 0.1)->where('user_id', $userId)->orderBy('id')->get();
